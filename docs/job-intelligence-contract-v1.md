...
AI_Job 采用“**宽采集、事实标注、下游裁决**”原则：

- 活跃、可核验的 2027 届正式岗位，不因当前候选人的专业、经历、目标城市或方向偏好在上游删除。
- 技术岗（软件/算法/硬件/机械/电气等）与财务/会计/审计/税务/金融/精算/法务等专业岗位都可保留；其专业硬门槛应写入 `major` / `jdEvidence`，最终 Eligibility / Professional Domain / Match 由 CareerPilot 判断。
- 销售、供应链、HR 等当前可能低优先的方向也不能因为候选人偏好在上游静默消失。
- 上游只允许按候选人无关条件排除：非 2027 届、实习/兼职、明确关闭/过期、伪公司/章节标题、无法达到生产源可信度要求的二手记录等。

### 来源边界

AI_Job 不实行“只保留高校校招”。生产源优先级为：

1. 公司官方校招官网 / ATS 职位级岗位；
2. 高校就业网官方发布、明确 2027 届且带公司官方投递入口的受控过渡记录；
3. 其它二手渠道主要用于发现与交叉取证，不能替代公司官方职位级来源。
...
4. **AI_Job 不按当前候选人画像删技术/财务/审计/金融/法务等专业岗位。** 上游只标事实门槛。
5. **公司官方职位级来源优先，高校渠道是补充而不是唯一来源。**
6. **同一岗位同步阶段与入库重算必须使用同一份 JD / `jdEvidence`。**
7. HC 必须保留 `job/program` scope；program 规模不能冒充岗位名额。
8. 三个仓库任何职责变化，都必须同步更新本契约，并用 CI/回归测试验证接口没有漂移。
