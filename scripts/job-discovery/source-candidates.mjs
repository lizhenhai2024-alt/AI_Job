...
export function buildDiscoveryQueue(records = [], audit = {}, { limit = 12, now = new Date(), forceRetry = false } = {}) {
  ...
  for (const record of records || []) {
    ...
    rows.push({
      ...
      careerUrlSeeded: Boolean(record.careerUrlSeeded),
      careerUrlVerifiedAt: record.careerUrlVerifiedAt || '',
      careerUrlEvidence: record.careerUrlEvidence || '',
      careerUrlGraduationYear: record.careerUrlGraduationYear || '',
      careerUrlCohortEvidence: record.careerUrlCohortEvidence || '',
