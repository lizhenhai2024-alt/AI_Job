...
const EXPERIENCE_WORDS = ['海外','运营','内容','项目','市场','电商','用户','数据','跨文化','营销','品牌','供应链','客户','管培','多语种','翻译'];
...
function cityOf(row = {}) {
  const raw = clean(row.workLocationName || row.workLocation || '');
  return CITY_NAMES.find((city) => raw.includes(city)) || raw || '待核';
}

function publishedAt(row = {}) {
  const ts = Number(row.releaseTime || 0);
  if (!ts) return '';
...
function directUrl(source, id) {
  // The campus listing is served under /campus; detail pages are client-side routed.
  return id ? `${source.url}#/grad/detail?id=${encodeURIComponent(id)}` : source.url;
}

export function parsePinduoduoJob(source, row = {}, now = new Date()) {
  const rawId = String(row.id || row.code || '');
  const title = clean(row.name || row.title || '');
  const duty = clean(row.jobDuty || row.description || '');
  const category = clean(row.jobName || '');
  const batch = clean(row.recruitTypeName || '');
  const tags = Array.isArray(row.labelList) ? row.labelList.map(clean).filter(Boolean) : [];
  const jobText = [title, category, batch, ...tags, duty].filter(Boolean).join('\n');
  ...
  return {
    id: `pinduoduo-${crypto.createHash('sha1').update(`${rawId}|${title}|${cityOf(row)}`).digest('hex').slice(0, 12)}`,
    company: source.company || '拼多多',
    title,
    roleFamily,
    city: cityOf(row),
    graduationYear: String(source.graduationYear || row.graduationYear || '2027'),
    skills,
    languages,
...
export async function searchPinduoduoJobs(profile, source, { fetcher = fetch, maxJobs, pageSize, maxPages, now = new Date() } = {}) {
  if (!source?.url) return { jobs: [], stats: { pages: 0, listed: 0, detailed: 0, keptJobs: 0, errors: 1, snapshotComplete: false, emptyResult: true } };
  const apiBase = 'https://careers.pddglobalhr.com';
  const listUrl = `${apiBase}/api/careers/api/recruit/position/list`;
  // The portal caps pageSize at 10 regardless of requested size; paginate by page number.
  ...
  try {
    for (let page = 1; page <= pageLimit && seen.size < jobLimit && listed < total; page++) {
      const payload = await postJson(fetcher, listUrl, { page, pageSize: size, t: null }, timeoutMs);
      pages++;
      ...
      for (const row of rows) {
        ...
        detailed++;
        try {
          const job = parsePinduoduoJob(source, row, now);
