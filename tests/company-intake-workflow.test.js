...
const refreshWorkflow = fs.readFileSync(new URL('../.github/workflows/refresh-campus-jobs.yml', import.meta.url), 'utf8');
...
test('company intake explicitly dispatches job refresh for a managed source', () => {
  ...
  assert.ok(workflow.includes('gh workflow run refresh-campus-jobs.yml'));
});

test('job refresh remains manually dispatchable for intake chaining', () => {
  assert.ok(refreshWorkflow.includes('workflow_dispatch:'), 'refresh-campus-jobs.yml must expose workflow_dispatch');
});
