...
- **官方源优先**：已接入 Moka、北森、安克官方公开 API、EcoFlow 官方飞书招聘 API；牛客用于广泛发现。
...
北森 zhiye 官方校招门户
安克创新官方公开 API
EcoFlow 官方飞书招聘 API（website-path=602892）
          ↓
    各数据源适配器
          ↓
2027 届别 / 截止 / 风险过滤
...
- **EcoFlow**：`jobs.ecoflow.com/602892` 对应 2027 校招站；通过飞书招聘 SaaS 匿名公开 `POST /api/v1/search/job/posts` 分页读取岗位，`website-path=602892` 固定到校招站，避免混入 generic `index` 社招目录。
